...
ParamInfoInd (individual parameters in Gibbs-sampler) and 
...
function calc_mu_vec(param_info::InitParameterInfo)
    mu_vec::Array{Float64, 1} = vcat(param_info.init_ind_param, param_info.init_error_param)
    return mu_vec
end
function calc_mu_vec(param_info::ParamInfoIndPre)
    mu_vec::Array{Float64, 1} = zeros(param_info.n_param)
    return mu_vec
end
function calc_mu_vec(param_info::ParamInfoInd)
    mu_vec::Array{Float64, 1} = param_info.init_ind_param
    return mu_vec
end
...
function init_mcmc(sampler::RandomWalk, param_info; step_length=0.1, cov_mat=I)
    ...
    step_length::Float64 = convert(Float64, step_length)
    cov_mat_use::Array{Float64, 2} = Array{Float64, 2}(I, (dim, dim))
    if cov_mat != I
        cov_mat_use::Array{Float64, 2} .= cov_mat
    end

    cov_mat_old::Array{Float64, 2} = deepcopy(cov_mat_use)
    mu_start::Array{Float64, 1} = calc_mu_vec(param_info)
...
function init_mcmc(sampler::AmSampler, param_info; cov_mat=I, gamma0=1.0, 
    ...
    gamma0::Float64 = convert(Float64, gamma0)
    alpha_power::Float64 = convert(Float64, alpha_power)
...
function init_mcmc(sampler::GenAmSampler, param_info; cov_mat=I, 
    ...
    gamma0::Float64 = convert(Float64, gamma0)
    alpha_power::Float64 = convert(Float64, alpha_power)
    alpha_target::Float64 = convert(Float64, alpha_target)

    # lambda = 2.38^2 / dim-of-sampling 
    log_lambda_start::Float64 = log_lambda
    mu_start::Array{Float64, 1} = calc_mu_vec(param_info)
...
function init_mcmc(sampler::RamSampler, param_info; cov_mat=I, 
    ...
    q_vec::Array{Float64, 1} = Array{Float64, 1}(undef, dim)
...
function init_mcmc_pilot(sampler::AmSampler, file_loc, rho; steps_before_update=100, 
    ...
    cov_mat::Array{Float64, 2} = readdlm(file_cov, '\t', Float64, '\n')
    mu = readdlm(file_mu, '\t', Float64, '\n')

    # Ensure correct type of all variables 
    dim = size(cov_mat)[1]
    mu_use::Array{Float64, 1} = Array{Float64, 1}(undef, dim)
    mu_use .= mu[:, 1]
    lambda::Float64 = 2.38^2 / dim
...
function init_mcmc_pilot(sampler::GenAmSampler, file_loc, rho; steps_before_update::T=100, 
    ...
    cov_mat::Array{Float64, 2} = readdlm(file_cov, '\t', Float64, '\n')
    mu = readdlm(file_mu, '\t', Float64, '\n')
    log_lambda = readdlm(file_lambda, '\t', Float64, '\n')

    # Ensure correct type of all variables 
    log_lambda_use::Array{Float64, 1} = [convert(Float64, log_lambda[1])]
...
function init_mcmc_pilot(sampler::RamSampler, file_loc, rho; steps_before_update=100, 
    ...
    # Read sampler components as floats 
    cov_mat = readdlm(file_cov, '\t', Float64, '\n')

    # Ensure correct type of all variables 
    dim = size(cov_mat)[1]

    # Vector of random-numbers 
    q_vec = Array{Float64, 1}(undef, dim)
            
    # General sampler variables 
    gamma0 = convert(Float64, gamma0)
    alpha_power = convert(Float64, alpha_power)
    alpha_target = convert(Float64, alpha_target)
...
function init_mcmc_pilot(sampler::RandomWalk, file_loc, rho; step_length=0.1, exp_tag=1, ind_id=1, dir_files="")
    ...
    cov_mat_old::Array{Float64, 2} = Array{Float64, 2}(I, (dim, dim))
    mean_vec::Array{Float64, 1} = Array{Float64, 1}(undef, dim)
...
function propose_parameters(x_prop, 
    ...
    normal_rand = randn(n_param)
    cov_mat::Array{Float64, 2} = deepcopy(mcmc_sampler.cov_mat)
    lambda::Float64 = exp(mcmc_sampler.log_lambda[1])
